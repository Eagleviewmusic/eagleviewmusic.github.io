...
(function () {
  ...
  function scheduleLoop(iteration, events, loopMs) {
    const base = loopAnchor + iteration * loopMs;

    events.forEach(ev => {
      const due = base + ev.tick * tickMs();
  ...
  function startPlayback() {
    ...
    const loopMs = totalTicks() * tickMs();
    if (!loopMs) return;
  ...
  function buildTrackHead(track, trackIndex) {
    ...
    mute.addEventListener('click', () => {
      ...
      if (isPlaying) startPlayback();   // rebuild the loop without it
      render();
    });
  ...
  function buildBeat(track, beatIndex, membership) {
    ...
    if (isQuarterRestBeat && beatIndex > 0 && hasRhythmToCopy(track, beatIndex - 1)) {
      notes.appendChild(buildBeatRepeatButton(track, beatIndex));
    }
  ...
  /* The beat-repeat simile mark quick-copy for quarter rests. Duplicates the
     rhythm structure of the preceding beat (Beat N - 1) into an empty beat.
  ...
  function hasRhythmToCopy(track, beatIndex) {
    if (beatIndex < 0 || beatIndex >= track.beats.length) return false;
    const beat = track.beats[beatIndex];
    if (!beat) return false;
    if (beat.cells.some(Boolean)) return true;
    if (beatIndex > 0 && isLinked(track, beatIndex - 1)) return true;
    return false;
  }

  function duplicateBeat(track, targetBeatIndex) {
    if (targetBeatIndex <= 0) return;
    const prevBeat = track.beats[targetBeatIndex - 1];
    if (!prevBeat) return;

    let newCells = prevBeat.cells.slice();
    if (!newCells.some(Boolean)) {
      newCells[0] = true;
    }

    track.beats[targetBeatIndex] = {
      slots: prevBeat.slots,
      cells: newCells
    };

    if (track.links) {
      delete track.links[targetBeatIndex];
    }
  ...
  function buildBeatRepeatButton(track, beatIndex) {
    const btn = document.createElement('button');
    btn.className = 'beat-repeat-btn';
    btn.title = 'Repeat the beat before this one';
    btn.setAttribute('aria-label', 'Repeat the beat before this one');
  ...
  document.addEventListener('keydown', e => {
    ...
    if (e.key === 'Escape') {
      /* Close the topmost thing that is open; only stop the music if
         nothing was. */
      const sheets = [titleModal, backupSheet, librarySheet]
      ...
      else if (!hadPopover && isPlaying) stopPlayback();
    }
  });
  ...
  /* ==================================================================
     ...
     ------------------------------------------------------------------
     html2canvas serialises the node without the page's stylesheet, and
     it cannot read a CSS transform reliably — so the grid is put back to
     its natural size for the shot and restored afterwards.
     ================================================================== */
  ...
  savePictureBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      downloadBlob(await new Promise(res => canvas.toBlob(res, 'image/png')),
                   (slugify(song.title) || 'ostinato') + '.png');
      ...
      grid.style.transform = savedTransform;
