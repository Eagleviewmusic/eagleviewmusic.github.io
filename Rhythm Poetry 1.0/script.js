(function() {
  ...
  function getBeatSubdivision(beatIndex, state = null) {
    ...
    if (s.timeSignatureDenominator === 8) {
      return 3;
    }
    return (s.beatSubdivisions && s.beatSubdivisions[beatIndex]) || 2;
  }

  function setBeatSubdivision(beatIndex, subdivision, state = null) {
    const s = state || getActiveState();
    if (s.timeSignatureDenominator === 8) return;
  ...
  function syncStates(fromMode) {
    if (fromMode === 'rhythm') {
      // Sync from rhythmState to poetryState
      poetryState.timeSignatureNumerator = rhythmState.timeSignatureNumerator;
      poetryState.timeSignatureDenominator = rhythmState.timeSignatureDenominator;
      poetryState.BPM = rhythmState.BPM;
      poetryState.hasPickupMeasure = rhythmState.hasPickupMeasure;
      poetryState.beatSubdivisions = { ...rhythmState.beatSubdivisions };
      poetryState.linkedBeats = { ...rhythmState.linkedBeats };
      poetryState.words = sanitizeWordsArray(poetryState.words);
      poetryState.canonical12 = mergeViewIntoCanonical(poetryState.canonical12, poetryState.words, poetryState);
      poetryState.words = fromCanonical12(poetryState.canonical12, poetryState);
    } else {
      // Sync from poetryState to rhythmState
      ...
      const totalBeats = Math.max(1, getTotalBeatsFromWords(poetryState.words, poetryState));
      rhythmState.beats = [];
      let wordCursor = 0;
      for (let b = 0; b < totalBeats; b++) {
        const S = getBeatSubdivision(b, poetryState);
        const bArr = [];
        for (let s = 0; s < S; s++) {
          bArr.push(isPositionActive(wordCursor + s, poetryState.words));
        }
        wordCursor += S;
  ...
  function loadSongById(songId) {
    ...
    if (song.poetryState) {
      ...
      poetryState.rawLyrics = poetryState.words.filter(w => w && w !== '-' && w.trim() !== '');
      poetryState.beatSubdivisions = song.beatSubdivisions ? { ...song.beatSubdivisions } : {};
    ...
    while (poetryState.canonical12.length < totalBeats * 12) {
      poetryState.canonical12.push('-');
    }
    poetryState.words = fromCanonical12(poetryState.canonical12, poetryState);
    ...
    if (song.rhythmState && song.rhythmState.beats && song.rhythmState.beats.length > 0) {
      ...
      for (let b = 0; b < totalBeats; b++) {
        ...
        const bArr = [];
        for (let s = 0; s < S; s++) {
          const w = poetryState.words[wordCursor + s];
