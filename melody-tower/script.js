...
function loadSettingsFromURL() {
  try {
    ...
    const timbreParam = params.get('timbre') || params.get('waveform');

    if (keyParam) {
      const keyMap = {
        'c': 0, 'c#': 1, 'db': 1, 'c♯': 1, 'd♭': 1,
        'd': 2, 'd#': 3, 'eb': 3, 'd♯': 3, 'e♭': 3,
        'e': 4, 'f': 5, 'f#': 6, 'gb': 6, 'f♯': 6, 'g♭': 6,
        'g': 7, 'g#': 8, 'ab': 8, 'g♯': 8, 'a♭': 8,
        'a': 9, 'a#': 10, 'bb': 10, 'a♯': 10, 'b♭': 10, 'b': 11
      };
      const cleanKey = keyParam.trim().toLowerCase();
      if (keyMap[cleanKey] !== undefined) {
        currentKeyIndex = keyMap[cleanKey];
      }
    }

    if (scaleParam && scaleDefinitions[scaleParam]) {
      currentScale = scaleParam;
    }

    if (timbreParam && waveforms.includes(timbreParam.toLowerCase())) {
      currentWaveformIndex = waveforms.indexOf(timbreParam.toLowerCase());
      currentWaveform = waveforms[currentWaveformIndex];
    }
  } catch (err) {
    console.warn('Could not parse URL parameters:', err);
  }
}

function hideShareOutput() {
  const container = document.getElementById('share-output-container');
...
function createControlsBar() {
  ...
  // Back to Hub Button (only shown if not in iframe)
  ...
  if (!isInsideIframe) {
    const backHubBtn = document.createElement('a');
    backHubBtn.className = 'back-hub-btn';
    backHubBtn.href = '../index.html';
  ...
  appTitle.textContent = 'Melody Tower';
  controlsBar.appendChild(appTitle);

  // 1. Key Section
  const keySection = document.createElement('div');
  keySection.className = 'sidebar-section';

  const keyTitle = document.createElement('div');
  keyTitle.className = 'sidebar-section-title';
  ...
  keyRow.appendChild(scaleControl);
  keySection.appendChild(keyTitle);
  keySection.appendChild(keyRow);

  // 2. Timbre Section
  const timbreSection = document.createElement('div');
  timbreSection.className = 'sidebar-section';

  const timbreTitle = document.createElement('div');
  timbreTitle.className = 'sidebar-section-title';
  timbreTitle.textContent = 'Timbre';
  ...
  // 3. Keyboard Layout Section
  const keyboardSection = document.createElement('div');
  keyboardSection.className = 'sidebar-section';

  const keyboardButton = document.createElement('button');
  keyboardButton.className = 'control-area keyboard-button';
  keyboardButton.tabIndex = 0;
  keyboardButton.setAttribute('aria-label', 'Computer keyboard layout');
  keyboardButton.textContent = 'Computer Keyboard layout';
  keyboardButton.id = 'keyboard-button';

  keyboardSection.appendChild(keyboardButton);

  // 4. Share Section
  const shareSection = document.createElement('div');
  shareSection.className = 'sidebar-section share-section';

  const shareButton = document.createElement('button');
  shareButton.className = 'control-area share-button';
  shareButton.id = 'share-button';
  shareButton.setAttribute('aria-label', 'Share this scale as a link');
  shareButton.textContent = 'Share this scale as a link';

  const shareOutputContainer = document.createElement('div');
  shareOutputContainer.className = 'share-output-container';
  shareOutputContainer.id = 'share-output-container';
  ...
  shareCopyBtn.className = 'share-copy-btn';
  shareCopyBtn.id = 'share-copy-btn';
  shareCopyBtn.textContent = 'Copy';

  shareOutputContainer.appendChild(shareInput);
  shareOutputContainer.appendChild(shareCopyBtn);

  shareSection.appendChild(shareButton);
  shareSection.appendChild(shareOutputContainer);

  controlsBar.appendChild(keySection);
  controlsBar.appendChild(timbreSection);
  controlsBar.appendChild(keyboardSection);
  controlsBar.appendChild(shareSection);

  return { keyButton, scaleControl, waveButton, keyboardButton, shareButton };
}
...
function setupControlEvents() {
  ...
  if (shareButton && shareContainer && shareInput && shareCopyBtn) {
    shareButton.addEventListener('click', () => {
      const currentKey = getEffectiveKeyName(currentKeyIndex, currentScale);
      const baseUrl = window.location.href.split('?')[0].split('#')[0];
      const shareUrl = `${baseUrl}?key=${encodeURIComponent(currentKey)}&scale=${encodeURIComponent(currentScale)}&timbre=${encodeURIComponent(currentWaveform)}`;
      ...
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(shareUrl).then(() => {
          ...
          setTimeout(() => {
            if (shareCopyBtn.textContent === 'Copied!') {
              shareCopyBtn.textContent = 'Copy';
